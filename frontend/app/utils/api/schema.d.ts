...
export interface components {
    schemas: {
        ...
        DownloadChapter: {
            ...
            /** @description The upgrade TARGET's own per-source attempt count (ProviderChapter.attempts) against THIS chapter — the numerator for badging the source the chapter is converging TO ("Asura Scans · 2/5"), the one actually being fetched, rather than the satisfier it replaces (whose `attempts` would read a misleading 0). Pairs with maxRetries exactly as `attempts` does, only for the target. 0 when the chapter has no upgrade target, or the target has no failed attempt yet; always describes the same source `upgradeTarget` names. */
