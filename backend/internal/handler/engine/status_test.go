...
// body stays within the cyclop gate; SourceCountsDTO carries a slice, so its
// counts are compared field-by-field rather than with ==.
...
func TestTopologyStatus_CountsAndGaps(t *testing.T) {
	...
	// Gaps: 2 extensions uncached, 1 url unresolved, 1 source whose read FAILED
	// (named). The old "sources without captured preferences" note is GONE.
