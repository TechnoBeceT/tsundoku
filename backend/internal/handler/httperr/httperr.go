...
// thin passthrough handlers (the engine settings + extensions proxies). Both
...
// Upstream maps a failure from a proxied upstream dependency — the download
// engine client, whether a transport error or a GraphQL rejection — to a 502
// Bad Gateway. The upstream is a separate service, so its failure is a gateway
// error rather than an internal one, and surfacing it as a 502 means a proxy
// handler never returns a false 200. The message is prefixed with "engine: " so
// the rendered error identifies which upstream failed.
func Upstream(err error) error {
	return echo.NewHTTPError(http.StatusBadGateway, "engine: "+err.Error())
}
