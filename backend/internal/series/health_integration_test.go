...
//   - a LIVE provider (provider "777", a numeric engine source id) whose source
//     is NOT in the engine's loaded set (its extension was uninstalled), and
//   - a disk-origin provider (provider "legacy-group", a display NAME) that is
//     likewise absent from the loaded set but must NEVER be flagged unavailable
//     (it was never an engine source — it fails series.IsLinkedProvider).
//
...
func seedUnavailableFixture(t *testing.T, ctx context.Context, db *ent.Client) string {
	...
	live := db.SeriesProvider.Create().
		SetSeriesID(s.ID).SetProvider("777").SetImportance(20).SaveX(ctx)
	dsk := db.SeriesProvider.Create().
		SetSeriesID(s.ID).SetProvider("legacy-group").SetImportance(10).SaveX(ctx)
...
// an engine source). Non-vacuous: drop the IsLinkedProvider guard and the
...
// empty-set guard treats an empty successful set as engine-not-ready.
...
// TestGetSeriesEmptyLoadedSetFlagsNothing proves the empty-set guard: an empty-
// but-successful loaded set (ok=true, len==0 — the engine restart/reload window
// where the endpoint answers before sources have loaded) is treated as
