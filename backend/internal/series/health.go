...
const (
	...
	// engine — its Suwayomi extension was uninstalled/removed, so nothing can
	// fetch from it. This is a DOMINANT fact (a missing extension outranks a
	// stored last_error or staleness): the fix is to reinstall the extension or
	// remove the source, not to wait for the next sweep.
...
type ProviderHealthInput struct {
	...
	// source) whose Suwayomi source id is no longer among the engine's
