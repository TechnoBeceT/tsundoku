...
func (_q *SeriesQuery) QueryProviders() *SeriesProviderQuery {
...
func (_q *SeriesQuery) QueryChapters() *ChapterQuery {
...
func (_q *SeriesQuery) WithProviders(opts ...func(*SeriesProviderQuery)) *SeriesQuery {
	query := (&SeriesProviderClient{config: _q.config}).Query()
...
func (_q *SeriesQuery) WithChapters(opts ...func(*ChapterQuery)) *SeriesQuery {
	query := (&ChapterClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *SeriesQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Series, error) {
	...
	if query := _q.withProviders; query != nil {
		if err := _q.loadProviders(ctx, query, nodes,
			func(n *Series) { n.Edges.Providers = []*SeriesProvider{} },
			func(n *Series, e *SeriesProvider) { n.Edges.Providers = append(n.Edges.Providers, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *SeriesQuery) loadProviders(ctx context.Context, query *SeriesProviderQuery, nodes []*Series, init func(*Series), assign func(*Series, *SeriesProvider)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(seriesprovider.FieldSeriesID)
	}
	query.Where(predicate.SeriesProvider(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(series.ProvidersColumn), fks...))
	}))
...
func (_q *SeriesQuery) loadChapters(ctx context.Context, query *ChapterQuery, nodes []*Series, init func(*Series), assign func(*Series, *Chapter)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Series)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(chapter.FieldSeriesID)
	}
	query.Where(predicate.Chapter(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(series.ChaptersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
