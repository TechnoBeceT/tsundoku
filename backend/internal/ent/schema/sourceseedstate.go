...
// one row per numeric Suwayomi source recording the OUTCOME of the last
...
//   - a source whose client.SourcePreferences READ errored (a real gap — a
//     setting we failed to harvest).
//
...
func (SourceSeedState) Fields() []ent.Field {
	return []ent.Field{
		...
		// source_id is the Suwayomi numeric source id this seed-state row belongs to
		...
		// client.SourcePreferences call for this source returned WITHOUT error.
		//
...
// Suwayomi source id, with no FK to any Tsundoku row.
