...
func (_q *CategoryQuery) QuerySeries() *SeriesQuery {
...
func (_q *CategoryQuery) WithSeries(opts ...func(*SeriesQuery)) *CategoryQuery {
	query := (&SeriesClient{config: _q.config}).Query()
...
func (_q *CategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Category, error) {
	...
	if query := _q.withSeries; query != nil {
		if err := _q.loadSeries(ctx, query, nodes,
			func(n *Category) { n.Edges.Series = []*Series{} },
			func(n *Category, e *Series) { n.Edges.Series = append(n.Edges.Series, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *CategoryQuery) loadSeries(ctx context.Context, query *SeriesQuery, nodes []*Category, init func(*Category), assign func(*Category, *Series)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(series.FieldCategoryID)
	}
	query.Where(predicate.Series(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(category.SeriesColumn), fks...))
	}))
