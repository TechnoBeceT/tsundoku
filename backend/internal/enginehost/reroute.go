...
// Rerouter is how the launcher + supervisor move a profile's sources between its
// own engine-host instance and the DEFAULT instance (GAP-114). It is the ONE seam
// between the OS-process side (this package) and the routing table
...
//   - Restore returns them to their base route only when every launcher-side
//     owner agrees: the instance is healthy and no retirement preparation is
//     waiting for replacement base-route publication.
//
