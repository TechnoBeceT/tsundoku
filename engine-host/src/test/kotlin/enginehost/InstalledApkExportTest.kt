...
class InstalledApkExportTest {
    ...
    fun `manager rejects a symlink even when its target is a regular file`() {
        InstalledApkFixture().use { fixture ->
            val outside = Files.createTempFile(testRoot, "installed-apk-outside", ".apk")
    ...
    @Test
    fun `RPC filesystem failure does not disclose the installed APK path`() {
        InstalledApkFixture().use { fixture ->
            fixture.apk.writeBytes("apk".toByteArray())
            Files.setPosixFilePermissions(fixture.apk, PosixFilePermissions.fromString("---------"))
            val server = RpcServer(fixture.loader, fixture.manager, port = 0, controlToken = CONTROL_TOKEN)
            server.start()
            try {
                val response = request(boundAddress(server).port, CONTROL_TOKEN)
                val body = response.body().toString(Charsets.UTF_8)

                assertEquals(502, response.statusCode())
                assertTrue(body.contains("installed APK is unavailable"), body)
                assertTrue(!body.contains(fixture.root.toString()), "response disclosed extension storage path: $body")
            } finally {
                Files.setPosixFilePermissions(fixture.apk, PosixFilePermissions.fromString("rw-------"))
                server.stop()
            }
        }
    }

    private inner class InstalledApkFixture : AutoCloseable {
        val root: Path = Files.createTempDirectory(testRoot, "fixture-")
