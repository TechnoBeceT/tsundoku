...
public class NewTransformer implements Transformer {
                ...
                if ("<init>".equals(ie.getName()) && "V".equals(ie.getRet())) {
                    ...
                    if (orgOps[0].vt == NEW) {
                        ...
                        if (newExpr != null) {
                            ...
                            // GAP-100 (d): take the instantiated type from the NEW expression, not
                            // ie.getOwner(). When R8/keiyoushi extensions collapse an allocation the
                            // <init> resolves to java/lang/Object.<init>, so getOwner() erases the
                            // concrete class to Object; newExpr.type preserves it (correct in both
                            // the normal and collapsed cases).
    ...
    void replace0(IrMethod method, Map<Local, TObject> init, int size) {
        ...
        for (TObject obj : init.values()) {
            ...
            Value[] orgOps = ie.getOps();
            Value[] nOps = Arrays.copyOfRange(orgOps, 1, orgOps.length);
            // GAP-100 (d): take the instantiated type from the object's own allocation
            // (a = NEW Abc), not ie.getOwner(). Here the invoke's receiver is still the Local,
            // not the NewExpr, so only obj.init.getOp2() yields it. getOwner() would erase a
            // collapsed allocation's concrete class to java/lang/Object; newExpr.type preserves it.
